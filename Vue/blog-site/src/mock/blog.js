...
mock('/api/blogtype', 'get', {
    code: 0, msg: "", "data|8-20": [{
        "id|+1": 0, name: "分类@id", "articleCount|5-500": 0, "order|0-20": 0
    }]
});

// 分页获取博客
mock(/\/api\/blog\?(?:page=.+|limit=.+|categoryId=.+){3}/, 'get', urlInfo => {
    const {
        limit
    } = urlParamsParse(urlInfo.url);

    return mock({
        code: 0, msg: "", data: {
            total: 457, [`rows|${limit}`]: [{
                id: '@id',
                title: "@ctitle(15, 30)",
                description: "@csentence(30, 50)",
                category: {
                    "id|1-10": 0, name: '分类@id'
                },
                "scanNumber|50-5000": 0,
                "commentNumber|5-500": 0,
                "thumb|1": ["@image(180x120, @color, '#FFF', @text)", null],
...
// 获取单个博客
mock(/\/api\/blog\?blogId=.+/, 'get', urlInfo => {
    const {
        blogId
    } = urlParamsParse(urlInfo.url);
    return mock({
        code: 0, msg: "", data: {
            ...
            toc: [{
                ...
                name: "简单请求", anchor: "article-md-title-2", children: [{
